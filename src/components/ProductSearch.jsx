  ...
  return (
    <div className="space-y-3">
            ...
            {(setResults.length > 0 || loadingSealed) && (
              ...
              >
                ...
                {loadingSealed && setResults.length === 0 && (
                  <div className="px-4 py-2 text-xs text-gray-600">Hľadám sety...</div>
                )}
              </Section>
            )}
            ...
            {(cardResults.length > 0 || loadingCards) && (
              ...
              >
                {cardResults.map((card) => (
                  ...
                  >
                    {card.image && (
                      <img src={card.image} alt={card.name} className="w-8 h-11 object-contain rounded shrink-0 opacity-90" />
                    )}
                    ...
                    <div className="shrink-0 text-right">
                      {card.cmPrice != null && <div className="text-xs text-blue-400">CM €{card.cmPrice}</div>}
                      {card.ebayPrice != null && <div className="text-xs text-yellow-500">TCG ~€{card.ebayPrice}</div>}
                    </div>
                  </li>
                ))}
                {loadingCards && cardResults.length === 0 && (
                  <div className="px-4 py-2 text-xs text-gray-600">Hľadám karty...</div>
                )}
              </Section>
            )}

            {!hasAnything && query.length >= 2 && !isLoading && (
              <div className="px-4 py-4 text-sm text-gray-600 text-center">Nič sa nenašlo</div>
            )}
          </div>
        )}
      </div>
      ...
      {showCustom ? (
        ...
        <button onClick={() => setShowCustom(true)} className="text-xs text-gray-600 hover:text-pkm-yellow transition-colors flex items-center gap-1">
...
function Section({ label, icon, children }) {
  return (
    <div>
      <div className="px-3 py-1.5 text-xs font-semibold text-gray-600 uppercase tracking-wider bg-black/20 flex items-center gap-1.5">
        {icon}
        {label}
      </div>
  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <div className="text-xs text-gray-500 truncate">{p.series ? `${p.series} · ` : ''}{p.set}{p.releaseDate ? ` · ${p.releaseDate.slice(0, 4)}` : ''}</div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <TypeBadge type={p.type} />
        {p.cmPrice != null
          ? <span className="text-xs text-blue-400">CM €{p.cmPrice}</span>
          : <span className="text-xs text-gray-700">cenu zadaj</span>
        }
      </div>
    </li>
  )
